...
**真正换"表情"(脸变)** 需要多张同角色不同表情的图。放进 `res/drawable/` 即可自动切换,缺失回退基础图:
`avatar_alarm`(警惕)、`avatar_happy`(开心/陪伴)、`avatar_listening`、`avatar_thinking`、`avatar_talking` —— 都用 `.png`。
> 你可以用同一 AI 工具生成"同一角色、警惕/担心表情"等几张,命名如上丢进去,危险时就是真表情。
