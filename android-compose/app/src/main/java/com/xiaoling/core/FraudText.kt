...
object FraudText {
    ...
    private val scriptPatterns = listOf(
        ...
        Regex("(?:视频|语音|声音|AI|换脸).{0,18}(?:儿子|女儿|孙子|孙女|亲友|领导).{0,24}(?:借钱|转账|汇款|急用钱)"),
        Regex("(?:关闭手机|关机|飞行模式|不要接电话).{0,18}(?:取现|现金|黄金|转账|汇款)"),
        Regex("(?:客服|平台|银行).{0,20}(?:退款|取消|扣费|账户异常).{0,24}(?:下载软件|人脸认证|刷脸|开启摄像头|银行卡)"),
    )
