...
## 部署后建议
- **接大模型(智能应用体)**:Kimi K3 是主推理模型,国内模型提供有界回退:
  - `MOONSHOT_API_KEY` — Kimi K3 对话、记忆推理和后台复杂任务
  - `DEEPSEEK_API_KEY` — 可选的国内文本回退
  ...
  MOONSHOT_API_KEY=在Kimi开放平台创建的Key
  XL_LLM_PROVIDERS=kimi,deepseek,qwen
  XL_DELEGATE_PROVIDER=kimi
  XL_DELEGATE_MODEL=kimi-k3
  ```
  通义实时音频与 Kimi K3 文本推理都在服务端配置,密钥不会打进 APK。
